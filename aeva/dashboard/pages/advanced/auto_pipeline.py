...
def render_pipeline_management():
    ...
    **核心功能**:
    ...
    st.markdown("### 📊 流水线可视化示例")

    selected_pipeline = st.selectbox(
        "选择流水线查看结构",
        ["ml_eval_pipeline", "data_quality_check", "model_benchmark"]
    )

    if selected_pipeline == "ml_eval_pipeline":
        st.markdown("""
        ```
        ┌─────────────────┐
        │  数据质量检查   │
    ...
    elif selected_pipeline == "data_quality_check":
        st.markdown("""
        ```
                    ┌─────────────────┐
                    │  数据加载验证   │
